...
1. **回路作成**: ESP32 の `GPIO 25` ピンと `GND` の間にスイッチを接続します。

   <div align="center">
   <img src="images/02_plcvsmicrocontroller_05.png" width="60%">
      <p>回路図</p>
   </div>

2. GPIO25 の状態を AD3 の Scope で観測します。
   ⚠！まだプログラムを書きません。
...
3. フラフラの周波数を観測せよ。  
AD3 のカーソル機能（赤い縦線を２箇所に出す機能）をつかって、フラフラの周波数を測定してください。なにか気づくことはありませんか？
...
* **【証拠写真】**:
    ...
    2. ** Shell の結果**: `utime.sleep` がない時に、1回押しただけで `print` が連打されてしまった Thonny の Shell の画面。

* **【考察】**: 

**「人間とマイコンの、時間の流れの違いを意識せよ」**

1. **第1の壁：マイコンが速すぎる問題**

* 人間が「最短」でボタンを連打しても、せいぜい1秒間に10回程度。1回押している時間は、短くても *100ms（0.1秒）* 長いと *150ms (0.15秒)* 程度ある。

* 一方、マイコンのループは1回 数$\mu$s（0.000001秒） レベル。

* **問い： もし sleep がなかったら、人間が「1回」押している 0.1秒の間に、マイコンは何回「ボタンが押されている（0である）」と判定してしまうだろうか？**

2. **第2の壁：接点のバウンド問題（チャタリング）**

* スイッチは物理的なバネと金属の板でできている。

* 押した瞬間、実は目に見えない速さで「微細な衝突と跳ね返り」を繰り返している。

* **問い： この「一瞬のガタガタ」をマイコンが律儀にすべて拾ってしまうと、プログラムはどう動いてしまうだろうか？**

### utime.sleep(0.1) の役割

utime.sleep(0.1) には２つの役割がある：

**役割１：** マイコンの速度を人間の指の動き（100ms程度）まで「わざと落とす」ことで、1回の押し動作を1回のイベントとして同期させる。（でも不完全）

**役割２：** スイッチがガタついている「最初の不安定な数ms」を無視し、状態が落ち着いた頃に次のチェックに行くことで、ノイズをやり過ごす。
