...
## 📘 Step 1：GPIO出力と「壊さないための」回路設計

第1回で行った「Lチカ」を、今日はエンジニアの視点で「設計」します。

### 1.1 オームの法則と制限抵抗の計算

マイコンのピンから流せる電流には限界があります（ESP32は約12〜20mA）。これを超えると内部回路が焼き切れます。一方、LED にも流せる電流の最大値 (定格電流) があります。
...
* **現場の判断**: 安全マージンと入手性を考え、本実習では **330 $\Omega$** を使用します。

> 💡エンジニアメモ：LED の電流制限抵抗を求めるときには $V_f$ に着目せよ。

### 1.2 【実習】電圧降下の実測

1.  LED回路（ESP32 → 330Ω抵抗 → LED → GND）を組む。GPIO 2 を使用する。
...
2.  AD3 のScope機能を使い、**「抵抗の両端」** の電圧差を測定せよ。(📸)
    GND (0V) 基準ではない。あくまで抵抗の両端。 
3.  2. の測定値から、実際に流れている電流値を計算せよ（$I = V / R$）。


---

## 📘 Step 2：マイコンと PC の通信 (UART通信) 〜「文字」の正体〜
...
* ちなみに USB も Universal Serial Bus の頭文字。シリアルなので、通信用の線は１本です。
...
3.  **AD3設定**: **Scope** を立ち上げ、Triggerを **「Falling（立ち下がり）」** に設定し、一瞬の波形をキャッチせよ。(📸) 
    ...
    **Trigger** というのは、その現象（この場合 Falling（立ち下がり））を検出したタイミングの波形を取得する、という機能です。**Falling** のほか、**Rising (立ち上がり)** などいろいろなタイミングの指定方法があります。

4.  **解析**: 捉えた波形を右から読み（LSBファースト）、`0100 0001` が隠れていることを目視で確認せよ。

---

## 📘 Step 3：ロジックアナライザと通信エラー

波形を目で追っていちいち 'A' かな？と確認するのは大変です。  
**「ロジックアナライザ」** 通称 **「ロジアナ」** を使ってみましょう。

### 3.1 WaveFormsの「Logic」機能

1.  WaveFormsの左メニューから **「Logic」** を選択。
2.  UART の設定を `115200 bps` にして 2.2 のプログラムを実行。
3.  AD3 が波形を自動的に「文字」へ復元（デコード）する様子を観察せよ。(📸) 
4. うまくできたら、自分の名前 (ローマ字) を出力するようにして、うまく復元できるか確認してみよう。

### 3.2 【トラブル演習】ボーレートの不一致（文字化けの再現）

1.  マイコン側は `115200` のまま、PC側（Thonny）の受信設定をわざと `9600` にズラして実行せよ。
2.  **現象**: シリアルモニタに化けた文字が表示されることを確認。
3.  **考察**: AD3 で観測した「1ビットあたりの時間の長さ（幅）」を、115200bpsと9600bpsで比較し、なぜ読み取れないのか考察せよ。(ちょっと難しい)

---

## 📘 Step 4：総合演習・対話型システムの構築
...
# 0 番ピンを「sw」という名前のオブジェクトを作る（プルアップは有効）
# このピンは入力ピン
sw = Pin(0, Pin.IN, Pin.PULL_UP) 
```

> **💡エンジニアメモ**: こうすることで、将来ピン番号が 2 から 3 とか 5 に変わっても、最初の一行を変えるだけで、`led.on()` というコードは修正せずに済みます（保守性の向上）。
...
* **ロジアナでの観測してみてよう**:
    AD3のロジックアナライザ画面で **RX（受信）側** を観測し、'Enter' を押した瞬間にどうなったか確認しよう。(📸) 
...
* **考察**: もし抵抗を入れずに「直結」したら、計算上は何 A 流れることになるか？

* **スクリーンショット**: print("A")` を実行したときの送信波形、ロジアナでのデコード (文字を復元した結果) の画面。
* **文字コードの理解**:　自分の名前 (ローマ字) の先頭１一文字を、16 進数と 2 進数 (8bit) で示せ。
