...
* 回路に積層セラミックコンデンサ（0.1μF程度）を挿入し、ノイズがどれだけ低減したかをAD3で再計測・実証すること。
...
* そこで、**特別なタイマー関数を使わず、これまでに習った「変数」と「if文」だけを組み合わせ**、「ループが50回まわる度（10ms × 50回 ＝ 500ms間隔）に1回だけOLEDの表示更新処理を行う」という**カウント式の疑似マルチタスク**を自力でアルゴリズムを考えて実装しなさい。
